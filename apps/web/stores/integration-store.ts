...
import type { IntegrationOut, IntegrationCreate, DiscoveryResult } from '@/types/api';
...
export const useIntegrationStore = create<IntegrationState>((set, get) => ({
    ...
    fetchIntegrations: async (homeId) => {
        ...
        try {
            ...
            set({ isLoading: false, error: err?.error?.message || err?.message || 'Failed to load integrations' });
        }
    },
    ...
    createIntegration: async (req) => {
        ...
        try {
            ...
            set({ isLoading: false, error: err?.error?.message || err?.message || 'Failed to create integration' });
            throw err;
        }
    },
    ...
    deleteIntegration: async (id) => {
        ...
        try {
            ...
            set({ isLoading: false, error: err?.error?.message || err?.message || 'Failed to delete integration' });
            throw err;
        }
    },
        ...
        try {
            ...
            set((prev) => ({
                integrations: prev.integrations.map((item) =>
                    item.id === id ? { ...item, last_sync_at: new Date().toISOString() } : item
                ),
            }));
            
            return res;
        } catch (err: any) {
            set({ isDiscovering: false, error: err?.error?.message || err?.message || 'Discovery scan failed' });
            throw err;
        }
    },
