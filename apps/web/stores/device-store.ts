...
interface DeviceState {
    ...
    updateDeviceStateInStore: (deviceId: string, newState: DeviceStateOut) => void;
}
...
export const useDeviceStore = create<DeviceState>((set, get) => ({
    ...
    fetchDevices: async (homeId) => {
        ...
        try {
            ...
            res.items.forEach((device) => {
                if (device.online) {
                    get().fetchDeviceState(device.id).catch(() => {});
                }
            });
        } catch (err: any) {
            set({ isLoading: false, error: err?.error?.message || err?.message || 'Failed to load devices' });
        }
    },
    ...
    turnOnDevice: async (deviceId) => {
        ...
        set((prev) => ({
            ...
            devices: prev.devices.map((d) => (d.id === deviceId ? { ...d, online: true } : d)),
        }));
    ...
    createDevice: async (req) => {
        ...
        try {
            ...
            set({ isLoading: false, error: err?.error?.message || err?.message || 'Failed to create device' });
            throw err;
        }
    },
    ...
    deleteDevice: async (deviceId) => {
        ...
        try {
            ...
            set({ isLoading: false, error: err?.error?.message || err?.message || 'Failed to delete device' });
            throw err;
        }
    },
    ...
    updateDeviceStateInStore: (deviceId, newState) => {
        set((prev) => ({
            ...
            devices: prev.devices.map((d) =>
                d.id === deviceId ? { ...d, online: true, last_seen: new Date().toISOString() } : d
            ),
        }));
    },
}));

function oldAttributes(state: DeviceStateOut | undefined, nextState: string): Record<string, any> {
