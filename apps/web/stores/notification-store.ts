...
export const useNotificationStore = create<NotificationState>((set, get) => ({
    ...
    fetchNotifications: async (homeId) => {
        ...
        try {
            ...
            set({ isLoading: false, error: err?.error?.message || err?.message || 'Failed to load notifications' });
        }
    },
    ...
    markAllAsRead: async (homeId) => {
        ...
        try {
            await Promise.all(unreadList.map((n) => notificationRepository.markRead(n.id)));
            set((prev) => ({
