...
export const useHomeStore = create<HomeState>((set, get) => ({
    ...
    fetchHomes: async () => {
        ...
        try {
            ...
            if (list.length > 0 && !get().activeHomeId) {
                const stored = typeof window !== 'undefined' ? localStorage.getItem('domus_active_home_id') : null;
                const matchesStored = stored && list.some((h) => h.id === stored);
                set({ activeHomeId: matchesStored ? stored : list[0].id });
            }
        } catch (err: any) {
            set({ isLoading: false, error: err?.error?.message || err?.message || 'Failed to load homes' });
        }
    },
    ...
    createHome: async (req) => {
        ...
        try {
            ...
            set({ isLoading: false, error: err?.error?.message || err?.message || 'Failed to create home' });
            throw err;
        }
    },
    ...
    updateHome: async (id, req) => {
        ...
        try {
            ...
            set({ isLoading: false, error: err?.error?.message || err?.message || 'Failed to update home' });
            throw err;
        }
    },
    ...
    deleteHome: async (id) => {
        ...
        try {
            ...
            set({ isLoading: false, error: err?.error?.message || err?.message || 'Failed to delete home' });
            throw err;
        }
    },
}));
