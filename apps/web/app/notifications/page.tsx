...
export default function NotificationsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Activity Log"
        description="Review system warnings, automation runs, and driver logs."
      >
        {unreadCount > 0 && (
          <button
            ...
            return (
              <div
                ...
                className={`rounded-3xl border p-5 backdrop-blur-sm relative flex items-start gap-4 transition hover:bg-card/30 ${
                  notif.read
                    ? "border-border/50 bg-card/10 opacity-70"
                    : "border-primary/25 bg-card/45 shadow-glow"
                }`}
              >
                <div
                  className={`rounded-xl border p-2.5 bg-background/80 flex-shrink-0 text-foreground/80`}
                >
                ...
                <div className="flex-1 min-w-0 space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-md text-[9px] font-bold border uppercase ${badgeColor}`}
                    >
