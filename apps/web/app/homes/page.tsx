...
export default function HomesPage() {
  ...
  const handleDeleteClick = async (id: string, name: string) => {
    ...
    if (
      confirm(
        `Are you sure you want to delete the home "${name}"? This deletes all associated rooms, integrations, and devices.`,
      )
    ) {
      try {
        await deleteHome(id);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Homes Switcher"
        description="Create and switch between home workspaces."
      >
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          <DialogTrigger asChild>
            <button
              onClick={() =>
                reset({ name: "", description: "", timezone: "Europe/Berlin" })
              }
          ...
          <DialogContent
            ...
            description="Configure parameters for your new smart home workspace."
          >
            <form
          ...
          return (
            <div
              ...
              className={`rounded-3xl border p-5 backdrop-blur-sm relative flex flex-col justify-between h-48 transition hover:bg-card/30 ${
                isActive
                  ? "border-primary/45 bg-primary/5 shadow-glow"
              ...
              <div>
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    <div
                      className={`rounded-xl border p-2 bg-background/80 ${isActive ? "text-primary border-primary/20" : "text-foreground/60 border-border/80"}`}
                    >
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent
          title="Edit Home Workspace"
          description="Update parameters for this smart home workspace."
        >
          <form
