...
export default function IntegrationsPage() {
    ...
    const handleDeleteClick = async (id: string, name: string) => {
        if (confirm(`Are you sure you want to delete "${name}"? This deletes connected device nodes.`)) {
            try {
        ...
        <div className="space-y-6">
            ...
            >
                <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                    ...
                    <DialogContent title="Add Driver Integration" description="Configure connector bindings for your smart home ecosystem.">
                        <form onSubmit={handleSubmit(handleCreateSubmit)} className="space-y-4 mt-2">
                            <div className="space-y-1">
                                <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Driver Profile Name</label>
                                <input
                                    type="text"
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1">
                                    <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Ecosystem Type</label>
                                    <select
                                ...
                                <div className="space-y-1">
                                    <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Connector IP</label>
                                    <input
                                        type="text"
                                        className="w-full rounded-xl border border-border bg-background/50 py-2.5 px-3.5 text-sm outline-none focus:border-primary"
                                        {...register('config.host')}
                                    />
                                    {errors.config?.host && <p className="text-xs text-rose-500 font-semibold">{errors.config.host.message}</p>}
                                </div>
                            </div>

                            <div className="space-y-1">
                                <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">API Token / Password (Opaque)</label>
                                <input
                                    type="password"
                                    placeholder="••••••••••••••••••••"
                                    className="w-full rounded-xl border border-border bg-background/50 py-2.5 px-3.5 text-sm outline-none focus:border-primary"
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {integrations.map((int) => {
                    const typeColor = typeLogoColors[int.type] || 'bg-muted text-muted-foreground border-border';
                    const connectedDevices = devices.filter((d) => d.integration_id === int.id);
                    ...
                    return (
                        <div
                            ...
                            className={`rounded-3xl border p-5 backdrop-blur-sm flex flex-col justify-between h-48 transition hover:bg-card/30 ${
                                isEnabled ? 'border-border/60 bg-card/25' : 'border-border/30 bg-card/5 opacity-55'
                            }`}
                        >
                            <div>
                                <div className="flex justify-between items-start">
                                    <div className="flex items-center gap-3">
                                        <div className={`rounded-xl border p-2.5 uppercase font-bold text-xs ${typeColor}`}>
                                            {int.type}
                                        </div>
                                        <div>
                                            <h3 className="font-semibold text-base leading-tight">{int.name}</h3>
                                ...
                                <div className="mt-4 grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                                    <div>
                                        <p className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground/60 leading-none">Nodes</p>
                                        <p className="mt-1.5 font-semibold text-foreground">{connectedDevices.length} Connected</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground/60 leading-none">Last Sync</p>
                                        <p className="mt-1.5 truncate">
                                            {int.last_sync_at
                                                ? new Date(int.last_sync_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
                                                : 'Never'}
                                        </p>
                                    </div>
                                </div>
                            ...
                            <div className="border-t border-border/40 pt-3 flex items-center justify-between mt-4">
                                <span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] font-bold border ${
                                    ...
                                    <span className={`h-1.5 w-1.5 rounded-full ${isEnabled ? 'bg-emerald-500 animate-pulse' : 'bg-muted-foreground'}`} />
                    ...
                    <div className="p-2 space-y-6">
                        ...
                        {scanningStep === 1 && (
                            <div className="flex flex-col items-center justify-center py-8 text-center space-y-4">
                                ...
                                <div>
                                    <p className="text-sm font-semibold">Broadcasting SSDP Discovery Packets...</p>
                                    <p className="text-xs text-muted-foreground mt-1">Checking subnet IPs for responsive ports.</p>
                                </div>
                            </div>
                        )}
                        ...
                        {scanningStep === 2 && (
                            <div className="flex flex-col items-center justify-center py-8 text-center space-y-4">
                                ...
                                <div>
                                    <p className="text-sm font-semibold">Registering discovered device nodes...</p>
                                    <p className="text-xs text-muted-foreground mt-1">Generating database entities & default attributes.</p>
                                </div>
                            </div>
                        )}
                            ...
                            <div className="space-y-4 mt-2">
                                <div className="rounded-2xl border border-emerald-500/25 bg-emerald-500/5 p-4 flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-muted-foreground mt-0.5">
                                            Found {discoveryResult.new_count} new accessories and identified {discoveryResult.existing_count} existing nodes.
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2 max-h-56 overflow-y-auto border border-border/50 rounded-xl p-2 bg-background/30">
                                    {discoveryResult.discovered.map((d, idx) => (
                                        <div key={idx} className="flex justify-between items-center text-xs p-2.5 rounded-lg border border-border/40 bg-card/40">
                                            <div className="truncate pr-4">
                                                <p className="font-semibold truncate">{d.name}</p>
                                                <p className="text-[10px] text-muted-foreground mt-0.5">{d.manufacturer} {d.model}</p>
                                            </div>
                                            <span className={`px-2 py-0.5 rounded-md text-[9px] font-bold border uppercase flex-shrink-0 ${
