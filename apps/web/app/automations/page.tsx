...
const automationSchema = z.object({
    ...
    actions: z.array(
        z.object({
            type: z.enum(['device.turn_on', 'device.turn_off', 'device.toggle', 'scene.activate', 'notification.send'] as const),
...
export default function AutomationsPage() {
    ...
    const { automations, createAutomation, deleteAutomation, triggerAutomation, toggleAutomation } = useAutomationStore();
    ...
    const { fields: condFields, append: condAppend, remove: condRemove } = useFieldArray({
        control,
        name: 'conditions',
    });

    const { fields: actFields, append: actAppend, remove: actRemove } = useFieldArray({
        control,
        name: 'actions',
    });
    ...
    return (
        <div className="space-y-6">
            <PageHeader title="Automations" description="Visual builder for smart local-first event triggers.">
                <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                    ...
                    <DialogContent title="Visual Automation Builder" description="Configure IF-THEN triggers and local actions.">
                        <form onSubmit={handleSubmit(handleCreateSubmit)} className="space-y-4 mt-2 max-h-[72vh] overflow-y-auto pr-1">
                            <div className="space-y-1">
                                <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Automation Name</label>
                                <input
                                    type="text"
                            ...
                            <div className="border border-border/60 bg-background/20 rounded-2xl p-4 space-y-3">
                                <p className="text-xs font-bold uppercase tracking-wider text-primary">1. IF Event Trigger</p>
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    {watchTriggerType === 'device_state' && (
                                        <>
                                            <div className="space-y-1">
                                                <label className="text-[10px] uppercase font-bold text-muted-foreground">Select Device</label>
                                                <select
                                                    className="w-full rounded-xl border border-border bg-background py-2 px-3 text-xs outline-none focus:border-primary cursor-pointer"
                                            ...
                                            <div className="space-y-1">
                                                <label className="text-[10px] uppercase font-bold text-muted-foreground">Trigger State</label>
                                                <select
                                                    className="w-full rounded-xl border border-border bg-background py-2 px-3 text-xs outline-none focus:border-primary cursor-pointer"
                                    ...
                                    {watchTriggerType === 'device_offline' && (
                                        <div className="space-y-1">
                                            <label className="text-[10px] uppercase font-bold text-muted-foreground">Target Device</label>
                                            <select
                                                className="w-full rounded-xl border border-border bg-background py-2 px-3 text-xs outline-none focus:border-primary cursor-pointer"
                                                {...register('trigger.device_id')}
                                            >
                                    ...
                                    {watchTriggerType === 'time' && (
                                        <div className="space-y-1">
                                            <label className="text-[10px] uppercase font-bold text-muted-foreground">Scheduled Time / Event</label>
                                            <input
                                                type="text"
                            ...
                            <div className="border border-border/60 bg-background/20 rounded-2xl p-4 space-y-3">
                                <div className="flex justify-between items-center">
                                    <p className="text-xs font-bold uppercase tracking-wider text-primary">2. AND Conditions (Optional)</p>
                                    <button
                                        type="button"
                                        onClick={() => condAppend({ field: 'lux', op: 'lt', value: '10' })}
                            ...
                            <div className="border border-border/60 bg-background/20 rounded-2xl p-4 space-y-3">
                                <div className="flex justify-between items-center">
                                    <p className="text-xs font-bold uppercase tracking-wider text-primary">3. THEN Actions (Chained)</p>
                                    <button
                                        type="button"
                                        onClick={() => actAppend({ type: 'device.turn_on', device_id: '' })}
                                        className="text-[11px] font-semibold text-primary hover:underline cursor-pointer"
                                    >
                                        + Add Action
                                    </button>
                                </div>
                                {errors.actions && <p className="text-xs text-rose-500 font-semibold">{errors.actions.message}</p>}

                                <div className="space-y-3">
                                    {actFields.map((field, idx) => {
                                        const type = watchActions[idx]?.type;
                                        return (
                                            <div key={field.id} className="flex gap-2 items-start border-b border-border/30 pb-3 last:border-b-0 last:pb-0">
                                                <div className="grid grid-cols-2 gap-2 flex-1">
                                                    <div className="space-y-1">
                                                        <label className="text-[9px] uppercase font-bold text-muted-foreground">Action Type</label>
                                                        <select
                                                        ...
                                                        >
                                                            <option value="device.turn_on">Turn On Device</option>
                                                            <option value="device.turn_off">Turn Off Device</option>
                                                            <option value="device.toggle">Toggle Device</option>
                                                            <option value="scene.activate">Activate Scene Preset</option>
                                                            <option value="notification.send">Send Toast Alert</option>
                                                        </select>
                                                    </div>

                                                    {type && type.startsWith('device.') && (
                                                        <div className="space-y-1">
                                                            <label className="text-[9px] uppercase font-bold text-muted-foreground">Target Accessory</label>
                                                            <select
                                                                className="w-full rounded-xl border border-border bg-background py-1.5 px-2 text-xs outline-none focus:border-primary cursor-pointer"
                                                                {...register(`actions.${idx}.device_id` as const)}
                                                            >
                                                    ...
                                                    {type === 'scene.activate' && (
                                                        <div className="space-y-1">
                                                            <label className="text-[9px] uppercase font-bold text-muted-foreground">Target Scene</label>
                                                            <select
                                                                className="w-full rounded-xl border border-border bg-background py-1.5 px-2 text-xs outline-none focus:border-primary cursor-pointer"
                                                                {...register(`actions.${idx}.scene_id` as const)}
                                                            >
                                                    ...
                                                    {type === 'notification.send' && (
                                                        <>
                                                            ...
                                                            <div className="space-y-1 col-span-2">
                                                                <label className="text-[9px] uppercase font-bold text-muted-foreground">Alert Body Message</label>
                                                                <input
                                                                    type="text"
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    {automations.map((auto) => {
                        ...
                        if (auto.trigger.type === 'device_state') {
                            const devName = devices.find((d) => d.id === auto.trigger.device_id)?.name || 'Device';
                        ...
                        return (
                            <div
                                ...
                                className={`rounded-3xl border p-5 backdrop-blur-sm flex flex-col justify-between transition hover:bg-card/30 ${
                                    auto.enabled ? 'border-border/60 bg-card/25' : 'border-border/30 bg-card/5 opacity-60'
                                }`}
                            >
                                <div className="flex justify-between items-start">
                                    <div className="flex items-center gap-3">
                                        <div className={`rounded-xl border p-2.5 bg-background/80 ${auto.enabled ? 'text-primary border-primary/20 shadow-sm animate-pulse-slow' : 'text-muted-foreground border-border/80'}`}>
                                            <Zap className="h-5 w-5" />
                                        </div>
                                        <div>
                                            <h3 className="font-semibold text-base leading-tight">{auto.name}</h3>
                                            <p className="text-[10px] text-muted-foreground mt-1">
                                                Runs {auto.actions.length} {auto.actions.length === 1 ? 'action' : 'actions'}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div className="scale-85">
                                            <Switch
                                                ...
                                                onCheckedChange={() => handleToggleRule(auto.id, auto.enabled)}
                                            />
                                        </div>
                                    </div>
                                </div>
