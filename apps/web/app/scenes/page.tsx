...
export default function ScenesPage() {
    ...
    const {
        ...
        defaultValues: { name: '', description: '', states: [{ device_id: '', state: 'off' }] },
    });
    ...
    return (
        <div className="space-y-6">
            <PageHeader title="Scenes" description="Orchestrate multiple device states to apply together.">
                <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                    <DialogTrigger asChild>
                        <button
                            onClick={() => reset({ name: '', description: '', states: [{ device_id: '', state: 'off' }] })}
                    ...
                    <DialogContent title="Visual Scene Builder" description="Save target states for multiple accessories.">
                        <form onSubmit={handleSubmit(handleCreateSubmit)} className="space-y-4 mt-2 max-h-[70vh] overflow-y-auto pr-1">
                            <div className="space-y-1">
                                <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Scene Name</label>
                                <input
                                    type="text"
                            ...
                            <div className="space-y-3 pt-2">
                                <div className="flex justify-between items-center">
                                    <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Target Device States</label>
                                    <button
                                        type="button"
                                ...
                                {errors.states && <p className="text-xs text-rose-500 font-semibold">{errors.states.root?.message || errors.states.message}</p>}
                        ...
                        >
                            <div className="flex justify-between items-start">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="font-semibold text-base leading-tight">{scene.name}</h3>
                                        <p className="text-xs text-muted-foreground mt-1">
                                            Controls {scene.states.length} {scene.states.length === 1 ? 'device' : 'devices'}
                                        </p>
                                    </div>
                                </div>
                                <button
