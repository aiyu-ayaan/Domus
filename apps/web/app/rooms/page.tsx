...
export default function RoomsPage() {
  ...
  const handleDeleteClick = async (id: string, name: string) => {
    if (
      confirm(
        `Are you sure you want to delete "${name}"? Devices inside this room will become unassigned.`,
      )
    ) {
      try {
        await deleteRoom(id);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Rooms"
        description="Manage physical partitions and group smart accessories."
      >
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          ...
          <DialogContent
            ...
            description="Group devices under a physical room in your home."
          >
            <form
              ...
              >
                ...
                <div className="border-t border-border/40 pt-3 flex items-center justify-between text-xs text-muted-foreground mt-4">
                  <span className="flex items-center gap-1.5">
                    <span
                      className={`h-2 w-2 rounded-full ${onlineDevCount > 0 ? "bg-cyan-500 animate-pulse" : "bg-muted-foreground"}`}
                    />
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent
          ...
          description="Modify details for this room partition."
        >
          <form
