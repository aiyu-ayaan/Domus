...
function DevicesPageContent() {
    ...
    useEffect(() => {
        ...
        setSearch('');
    }, [roomQuery, setSelectedRoomId, setSelectedType, setSelectedStatus, setSearch]);
    ...
    }, [activeHomeId, selectedRoomId, selectedType, selectedStatus, search, fetchDevices]);
    ...
    return (
        <div className="space-y-6">
            <PageHeader title="Devices" description="Toggle plug switches, dim lights, and inspect device telemetry.">
                <div className="flex items-center gap-1.5 border border-border bg-background/50 p-1 rounded-xl">
                    <button
                        onClick={() => setIsGridView(true)}
                        className={`rounded-lg p-1.5 transition cursor-pointer ${
                            isGridView ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-accent/40'
                        }`}
                        title="Grid view"
                    >
            ...
            <div className="grid gap-3 sm:flex sm:items-center sm:justify-between border-b border-border/40 pb-4 mb-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <select
                        ...
                        onChange={(e) => setSelectedRoomId(e.target.value === 'all' ? null : e.target.value)}
            ...
            {filteredDevices.length === 0 ? (
                ...
                <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
                    {filteredDevices.map((dev) => {
                        ...
                        const roomName = rooms.find((r) => r.id === dev.room_id)?.name || 'Unassigned';
                        
                        const isToggleable = ['light', 'plug', 'switch', 'lock'].includes(dev.device_type);
                        ...
                        return (
                            <div
                                ...
                                className={`rounded-3xl border p-5 backdrop-blur-sm flex flex-col justify-between h-48 transition hover:bg-card/30 ${
                                    dev.online ? 'border-border/60 bg-card/20' : 'border-border/40 bg-card/5 opacity-60'
                                }`}
                            >
                                <div className="flex justify-between items-start">
                                    <div className="flex items-center gap-3">
                                        <div className={`rounded-xl border p-2 bg-background/80 ${dev.online ? 'text-primary border-primary/15 shadow-sm' : 'text-muted-foreground border-border/80'}`}>
                                            <Icon className="h-5 w-5" />
                                        </div>
                                        <div className="truncate max-w-[120px] sm:max-w-[140px]">
                                            <h3 className="font-semibold text-sm truncate leading-tight">{dev.name}</h3>
                                            <p className="text-[10px] text-muted-foreground leading-none mt-1.5">{roomName}</p>
                                        </div>
                                    </div>
                                ...
                                <div className="my-3">
                                    {dev.online ? (
                                        <div className="text-xs text-muted-foreground">
                                            {dev.device_type === 'plug' && state?.attributes?.current_consumption !== undefined && (
                                                <p className="font-semibold text-foreground/90">
                                                    Load: {state.attributes.current_consumption} W
                                                </p>
                                            )}
                                            {dev.device_type === 'light' && state?.attributes?.brightness !== undefined && (
                                                <p className="font-semibold text-foreground/90">
                                            ...
                                            {dev.device_type === 'thermostat' && (
                                                <p className="font-semibold text-foreground/90">
                                                    Temp: {state?.state || '21.5'}°C → {state?.attributes?.target_temperature || '22.0'}°C
                                                </p>
                                            )}
                                            {dev.device_type === 'lock' && (
                                                <p className="font-semibold text-foreground/90">
                                                    Status: {state?.state === 'closed' ? 'Locked' : 'Unlocked'}
                                                </p>
                                            )}
                                            {dev.device_type === 'sensor' && state?.attributes?.lux !== undefined && (
                                                <p className="font-semibold text-foreground/90">
                                            ...
                                            {!['plug', 'light', 'thermostat', 'lock', 'sensor', 'camera'].includes(dev.device_type) && (
                                                <p className="font-semibold text-foreground/90">
                                        ...
                                        <p className="text-xs text-rose-500 font-semibold">Disconnected</p>
                                    )}
                                </div>
                                ...
                                <div className="border-t border-border/40 pt-2 flex items-center justify-between mt-auto">
                                    <div className="flex items-center gap-1.5">
                                        <span className={`h-2 w-2 rounded-full ${dev.online ? 'bg-cyan-500 animate-pulse' : 'bg-muted-foreground'}`} />
                                    ...
                                    return (
                                        <tr
                                            ...
                                            className={`border-b border-border/40 transition hover:bg-accent/20 ${
                                                dev.online ? 'text-foreground' : 'text-muted-foreground opacity-60'
                                            }`}
                                        >
                                            ...
                                            <td className="p-4">
                                                <span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] font-semibold border ${
                                                    ...
                                                    <span className={`h-1.5 w-1.5 rounded-full ${dev.online ? 'bg-cyan-500 animate-pulse' : 'bg-muted-foreground'}`} />
                                                    {dev.online ? 'Online' : 'Offline'}
                                                </span>
                                            </td>
