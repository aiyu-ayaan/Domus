...
export default function DeviceDetailPage() {
    ...
    const { deviceStates, updateDeviceStateInStore, toggleDevice, deleteDevice } = useDeviceStore();
    ...
    if (isLoading || !device) {
        return (
            <div className="flex h-[60vh] flex-col items-center justify-center">
                ...
                <p className="mt-4 text-sm text-muted-foreground animate-pulse-slow">Querying device logs...</p>
            </div>
        );
    }
    ...
    const handleDeleteDevice = async () => {
        if (confirm(`Are you sure you want to delete "${device.name}"? This removes the hardware and its historical records.`)) {
            try {
    ...
    const chartData = [...history]
        .reverse()
        .map((h) => {
            const time = new Date(h.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' });
            let val = 0;
        ...
        <div className="space-y-6">
            ...
            <Tabs defaultValue="overview">
                ...
                <TabsContent value="overview">
                    <div className="grid gap-6 md:grid-cols-3">
                        ...
                        <div className="rounded-3xl border border-border/60 bg-card/25 p-5 md:col-span-2 backdrop-blur-sm space-y-6">
                            <div>
                                <h3 className="font-semibold text-base">Quick Control Actions</h3>
                                <p className="text-xs text-muted-foreground mt-1">Directly adjust states on the hardware.</p>
                            </div>
                            ...
                            {device.online ? (
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between rounded-2xl border border-border/50 bg-background/30 p-4">
                                        ...
                                        {['light', 'plug', 'switch', 'lock'].includes(device.device_type) ? (
                                            <Switch checked={isChecked} onCheckedChange={handleQuickToggle} />
                                        ) : (
                                            <span className="text-xs text-muted-foreground">Toggle not supported</span>
                                        )}
                                    </div>
                                    ...
                                    {device.device_type === 'thermostat' && (
                                        <div className="rounded-2xl border border-border/50 bg-background/30 p-4 space-y-3">
                                            ...
                                            <input
                                                ...
                                                max="28"
                                                step="0.5"
                                                defaultValue={state?.attributes?.target_temperature || '22'}
                                                onChange={async (e) => {
                                                    const targetVal = parseFloat(e.target.value);
                                                    try {
                                                        const res = await deviceRepository.update(device.id, {
                                                            online: true,
                                                        });
                                                        // Update attributes
                                                        const curState = await deviceRepository.getState(device.id);
                                                        const updatedState = await deviceRepository.turnOn(device.id); // turn-on updates
                                                        updatedState.attributes.target_temperature = targetVal;
                                            ...
                                            <input
                                                ...
                                                onChange={async (e) => {
                                                    ...
                                                    try {
                                                        ...
                                                        updateDeviceStateInStore(device.id, { ...curState });
                                                    } catch {}
                                                }}
                                                className="w-full h-1.5 rounded-lg bg-muted border-none outline-none accent-primary cursor-pointer"
                                            />
                        ...
                        <div className="rounded-3xl border border-border/60 bg-card/25 p-5 backdrop-blur-sm space-y-4 text-sm">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground mt-1">Technical hardware details.</p>
                            </div>
                            <div className="divide-y divide-border/40 space-y-3">
                                ...
                                <div className="flex justify-between py-1.5 pt-3">
                                    ...
                                    <span className="font-mono text-xs truncate max-w-[150px]">{device.external_id}</span>
                                </div>
                                <div className="flex justify-between py-1.5 pt-3">
                ...
                <TabsContent value="activity">
                    <div className="rounded-3xl border border-border/60 bg-card/25 p-5 backdrop-blur-sm space-y-4">
                        <div>
                            <h3 className="font-semibold text-base">State Change Logs Feed</h3>
                            <p className="text-xs text-muted-foreground mt-1">Logs pushed over websocket connections.</p>
                        </div>
                                    ...
                                    <tbody>
                                        {history.map((h, idx) => (
                                            <tr key={idx} className="border-b border-border/40 text-muted-foreground">
                                                ...
                                                <td className="p-3">
                                                    <span className="capitalize font-semibold text-foreground">{h.state}</span>
                                                </td>
                ...
                <TabsContent value="settings">
                    <div className="grid gap-6 md:grid-cols-3">
                        <div className="rounded-3xl border border-border/60 bg-card/25 p-5 md:col-span-2 backdrop-blur-sm space-y-5">
                            <div>
                                <h3 className="font-semibold text-base">Edit Device Settings</h3>
                                <p className="text-xs text-muted-foreground mt-1">Rename or reassign your device.</p>
                            </div>
                            ...
                            <form onSubmit={handleSubmit(handleSettingsSubmit)} className="space-y-4">
                                ...
                                <div className="space-y-1">
                                    <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Room Assignment</label>
                                    <select
                        ...
                        <div className="rounded-3xl border border-rose-500/25 bg-rose-500/5 p-5 backdrop-blur-sm space-y-4">
                            <div>
                                <h3 className="font-semibold text-base text-rose-500">Danger Zone</h3>
                                <p className="text-xs text-muted-foreground mt-1">Actions cannot be reverted.</p>
                            </div>
                            <p className="text-xs text-muted-foreground leading-relaxed">
                                Removing a device detaches it from room scopes and deletes all history logging records.
                            </p>
                            <button
                                type="button"
