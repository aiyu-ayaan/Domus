...
import type { IntegrationOut, IntegrationCreate, IntegrationUpdate, DiscoveryResult, DiscoveredDevice, DeviceOut } from '@/types/api';
...
export class MockIntegrationRepository implements IIntegrationRepository {
    ...
    public async listAvailable(): Promise<string[]> {
        await this.delay(50);
        return ['tapo', 'xiaomi', 'tuya', 'mqtt', 'matter', 'zigbee', 'shelly', 'sonoff'];
    }
    ...
    public async get(id: string): Promise<IntegrationOut> {
        ...
        if (!integration) {
            throw { error: { code: 'not_found', message: 'Integration not found', details: null } };
        }
        return integration;
    }
    ...
    public async update(id: string, req: IntegrationUpdate): Promise<IntegrationOut> {
    ...
    public async delete(id: string): Promise<void> {
        ...
        const integrations = mockDb.get('integrations');
        mockDb.set('integrations', integrations.filter((i) => i.id !== id));

        // Delete or orphan devices connected to this integration
        const devices = mockDb.get('devices');
        mockDb.set('devices', devices.filter((d) => d.integration_id !== id));
    }
    ...
    public async discover(id: string): Promise<DiscoveryResult> {
        ...
        const candidates: Record<string, DiscoveredDevice[]> = {
            tapo: [
                { external_id: 'tapo-l900-02', name: 'TP-Link Tapo LED Strip Room 2', device_type: 'light', manufacturer: 'TP-Link', model: 'Tapo L900-5', serial_number: 'TAPO-L900-4491', attributes: { brightness: 100 }, already_registered: false },
                { external_id: 'tapo-p110-02', name: 'TP-Link Smart Plug Coffee Machine', device_type: 'plug', manufacturer: 'TP-Link', model: 'Tapo P110', serial_number: 'TAPO-P110-2394', attributes: { power: 0 }, already_registered: false },
            ],
            tuya: [
                { external_id: 'tuya-bulb-02', name: 'Tuya Ceiling Light Living Room', device_type: 'light', manufacturer: 'Tuya', model: 'Bulb RGBW', serial_number: 'TUYA-BULB-7741', attributes: { brightness: 80 }, already_registered: false },
            ],
            xiaomi: [
                { external_id: 'xiaomi-temp-01', name: 'Xiaomi Temperature Humidistat', device_type: 'sensor', manufacturer: 'Xiaomi', model: 'Mijia Sensor', serial_number: 'XIAO-TEMP-9922', attributes: { temperature: 22.4, humidity: 48 }, already_registered: false },
            ],
            zigbee: [
                { external_id: 'zigbee-leak-01', name: 'Zigbee Water Leak Sensor', device_type: 'sensor', manufacturer: 'Aqara', model: 'Leak Sensor', serial_number: 'AQ-LEAK-3829', attributes: { battery: 98, water_leak: false }, already_registered: false },
            ],
            matter: [
                { external_id: 'matter-lock-02', name: 'Matter Smart Lock Backdoor', device_type: 'lock', manufacturer: 'Schlage', model: 'Encode Plus', serial_number: 'SCH-LOCK-1188', attributes: { battery: 94, locked: true }, already_registered: false },
            ],
        };

        const discovered = candidates[integration.type] || [
            { external_id: `${integration.type}-device-01`, name: `${integration.name} Device 01`, device_type: 'switch', manufacturer: 'Generic', model: 'Smart Switch', serial_number: 'GEN-SW-0001', attributes: {}, already_registered: false }
        ];
        ...
        discovered.forEach((item) => {
            const exists = devices.find((d) => d.home_id === integration.home_id && d.external_id === item.external_id);
            if (exists) {
        ...
        if (newDevicesToRegister.length > 0) {
            ...
            newDevicesToRegister.forEach((d) => {
                const defaultState = d.device_type === 'thermostat' ? '21.0' : d.device_type === 'lock' ? 'closed' : 'off';
                states[d.id] = {
