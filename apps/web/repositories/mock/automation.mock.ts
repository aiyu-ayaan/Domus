...
export class MockAutomationRepository implements IAutomationRepository {
    ...
    public async get(id: string): Promise<AutomationOut> {
        ...
        if (!auto) {
            throw { error: { code: 'not_found', message: 'Automation not found', details: null } };
        }
        return auto;
    }
    ...
    public async delete(id: string): Promise<void> {
        ...
        const automations = mockDb.get('automations');
        mockDb.set('automations', automations.filter((a) => a.id !== id));
    }
            ...
            for (const cond of auto.conditions) {
                ...
                switch (cond.op) {
                    ...
                    case 'lte':
                        ...
                        break;
                    case 'in':
                        if (!Array.isArray(cond.value) || !cond.value.includes(ctxVal)) matched = false;
                        break;
                    default:
                        matched = false;
                }
