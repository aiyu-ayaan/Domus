...
import type { DeviceOut, DeviceCreate, DeviceUpdate, DeviceStateOut, Page } from '@/types/api';
...
export class MockDeviceRepository implements IDeviceRepository {
    ...
    public async list(params: {
        ...
        await this.delay();
        let list = mockDb.get('devices').filter((d) => d.home_id === params.home_id);
    ...
    public async get(id: string): Promise<DeviceOut> {
        ...
        if (!device) {
            throw { error: { code: 'not_found', message: 'Device not found', details: null } };
        }
        return device;
    }
    ...
    public async create(req: DeviceCreate): Promise<DeviceOut> {
        ...
        const dup = devices.find((d) => d.home_id === req.home_id && d.external_id === req.external_id);
        if (dup) {
            throw { error: { code: 'conflict', message: 'Device external_id conflict', details: null } };
        }
        ...
        const defaultState = req.device_type === 'thermostat' ? '21.0' : req.device_type === 'lock' ? 'closed' : 'off';
        const newState: DeviceStateOut = {
            id: 'state-' + Math.random().toString(36).substr(2, 9),
            device_id: newDevice.id,
            state: defaultState,
            attributes: req.device_type === 'light' ? { brightness: 100 } : req.device_type === 'thermostat' ? { target_temperature: 21.0 } : {},
            created_at: new Date().toISOString(),
        };
    ...
    public async delete(id: string): Promise<void> {
        ...
        const devices = mockDb.get('devices');
        mockDb.set('devices', devices.filter((d) => d.id !== id));
    ...
    public async getState(id: string): Promise<DeviceStateOut> {
        ...
        if (!state) {
            ...
            const defaultState = device.device_type === 'thermostat' ? '21.0' : device.device_type === 'lock' ? 'closed' : 'off';
    ...
    public async getHistory(id: string, limit?: number, offset?: number): Promise<DeviceStateOut[]> {
        await this.delay(100);
    ...
    private async setDeviceState(id: string, stateString: string): Promise<DeviceStateOut> {
        await this.delay(100);
        const devices = mockDb.get('devices');
        ...
        if (updatedDevices[deviceIdx].device_type === 'plug') {
            newAttributes.current_consumption = stateString === 'on' ? +(10 + Math.random() * 90).toFixed(1) : 0;
            newAttributes.energy_today = +( (newAttributes.energy_today || 0.42) + (stateString === 'on' ? 0.01 : 0) ).toFixed(3);
        }
