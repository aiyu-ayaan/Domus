    ...
    if (useMock) {
      ...
      const interval = setInterval(() => {
        ...
        if (rand < 0.4) {
          ...
          const devices = useDeviceStore
            .getState()
            .devices.filter((d) => d.online && d.home_id === homeId);
          if (devices.length === 0) return;

          const randomDevice =
            devices[Math.floor(Math.random() * devices.length)];
          const devState =
            useDeviceStore.getState().deviceStates[randomDevice.id];
          ...
          const devices = useDeviceStore
            .getState()
            .devices.filter((d) => d.home_id === homeId);
          if (devices.length === 0) return;
          ...
          if (selectedType === "automation_failed") {
            ...
            body =
              'Rule "Morning Routine Setup" could not turn on Master Bedroom thermostat.';
          ...
          useNotificationStore
            .getState()
            .addNotificationInStore(newNotification);
      ...
      return () => {
        window.removeEventListener(
          "domus_mock_ws_broadcast",
          handleLocalBroadcast,
        );
  ...
  const handleSocketMessage = (msg: any) => {
    ...
    switch (msg.type) {
      ...
      case "device.online_changed": {
        ...
        if (!online) {
          const devName =
            useDeviceStore.getState().devices.find((d) => d.id === device_id)
              ?.name || "Device";
